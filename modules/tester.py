...
def test_achievements(project_name, user_id, attempts_table_name): 
    ...
    if len(user_score_evolution) < 1 or user_score_evolution is None < 0:
        ...
        if len(user_score_evolution_df_month) < 1 or user_score_evolution_df_month is None < 0:
            ...
            col3.metric(label="# % tests with score >= 80", value = str(round((user_score_evolution_df_month['score'] >= 80).sum()/user_score_evolution_df_month.shape[0])))
        ...
        col3.metric(label="# % tests with score >= 80", value = str(round((user_score_evolution_df['score'] >= 80).sum()/user_score_evolution_df.shape[0])))
