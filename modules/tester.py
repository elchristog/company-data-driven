...
def test_achievements(project_name, user_id, attempts_table_name): 
    ...
    if len(user_score_evolution) < 1 or user_score_evolution is None < 0:
        ...
        col1.metric(label="# % tests with score > 80", value = str((user_score_evolution['score'] > 80).sum()/len(user_score_evolution)), delta= 0)
        col1.metric(label="# Avg days between tests", value = str((user_score_evolution['score'] > 80).sum()/len(user_score_evolution)), delta= 0)
