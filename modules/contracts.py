...
def contracts_crm_show_metrics(project_name):
    contracts_crm_oportunities = uc.run_query_1_h(f"SELECT DISTINCT * FROM (SELECT CONCAT(tawl.phone_indicator,tawl.phone_number) AS full_phone_number, (COALESCE(DATE_DIFF(CURRENT_DATE(), last_contact_df.last_contact_date, DAY), 99999) - CASE WHEN last_contact_df.last_user_status LIKE '%15%' THEN 15 WHEN last_contact_df.last_user_status LIKE '%30%' THEN 30 WHEN last_contact_df.last_user_status LIKE '%60%' THEN 60 ELSE 0 END) AS days_since_last_contact FROM `company-data-driven.enfermera_en_estados_unidos.traffic_analytics_whatsapp_leads` AS tawl INNER JOIN `company-data-driven.enfermera_en_estados_unidos.traffic_analytics_groupal_session_assistance` AS tagsa ON tawl.id = tagsa.traffic_analytics_whatsapp_lead_id LEFT OUTER JOIN (SELECT traffic_analytics_whatsapp_leads_id, LAST_VALUE(user_status) OVER(PARTITION BY traffic_analytics_whatsapp_leads_id ORDER BY contact_date) AS last_user_status, LAST_VALUE(contact_date) OVER(PARTITION BY traffic_analytics_whatsapp_leads_id ORDER BY contact_date) AS last_contact_date,  ROW_NUMBER() OVER(PARTITION BY traffic_analytics_whatsapp_leads_id ORDER BY contact_date DESC) AS row_number_contact FROM `company-data-driven.enfermera_en_estados_unidos.contract_crm_log`) AS last_contact_df ON (tawl.id = last_contact_df.traffic_analytics_whatsapp_leads_id AND last_contact_df.row_number_contact = 1) WHERE tawl.id NOT IN (SELECT traffic_analytics_whatsapp_leads_id FROM `company-data-driven.enfermera_en_estados_unidos.contracts`) AND (last_contact_df.last_user_status LIKE '%active%' OR last_contact_df.last_user_status IS NULL) AND COALESCE(DATE_DIFF(CURRENT_DATE(),last_contact_df.last_contact_date, DAY), 99999) > 3 AND tagsa.status = 'assistant' ORDER BY (COALESCE(DATE_DIFF(CURRENT_DATE(), last_contact_df.last_contact_date, DAY), 99999) - CASE WHEN last_contact_df.last_user_status LIKE '%15%' THEN 15 WHEN last_contact_df.last_user_status LIKE '%30%' THEN 30 WHEN last_contact_df.last_user_status LIKE '%60%' THEN 60 ELSE 0 END) DESC;")
