...
def customer_creation(user_id_customer_creation, project_id, project_name): 
    ...
    user_country = st.selectbox(
        ...
        options = ['colombia', 'united states', 'el salvador', 'mexico', 'venezuela', 'costa rica', 'chile', 'peru', 'espana'],
        index = None
    )
    ...
    if len(checking_username) < 1 or checking_username is None:
        ...
        if selected_username is not None:
           ...
           SELECT 
                ...
                ROUND(CAST(c.contract_total_value AS NUMERIC), 0) - ROUND(CAST(total_payments.total_paid AS NUMERIC) AS current_debt, 
