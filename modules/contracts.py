...
def contracts_show_metrics(project_name):
  ...
  if len(dates_groupal_meeting) < 1:
      ...
      df_conversion = pd.DataFrame(uc.run_query_1_h(f"SELECT COALESCE(groupal_session_assistants.date, contracts_counts.date) AS date, COALESCE(num_assistants_groupal_session,0) AS num_assistants_groupal_session, COALESCE(num_contracts,0) AS num_contracts FROM (SELECT meeting_date AS date, COUNT(id) AS num_assistants_groupal_session FROM `company-data-driven.enfermera_en_estados_unidos.traffic_analytics_groupal_session_assistance` GROUP BY meeting_date
) AS groupal_session_assistants FULL OUTER JOIN (SELECT contract_date AS date, COUNT(id) AS num_contracts FROM `company-data-driven.enfermera_en_estados_unidos.contracts` GROUP BY contract_date) contracts_counts ON groupal_session_assistants.date = contracts_counts.date;"))
      st.write(df_conversion)

#       df_conversion = pd.DataFrame(uc.run_query_1_h(f"SELECT COALESCE(groupal_session_assistants.date, contracts_counts.date) AS date, COALESCE(num_assistants_groupal_session, 0) AS num_assistants_groupal_session, COALESCE(num_contracts, 0) AS num_contracts FROM (SELECT meeting_date AS date, COUNT(id) AS num_assistants_groupal_session FROM `company-data-driven.{project_name}.traffic_analytics_groupal_session_assistance` GROUP BY meeting_date
# ) AS groupal_session_assistants FULL OUTER JOIN (SELECT contract_date AS date, COUNT(id) AS num_contracts FROM `company-data-driven.{project_name}.contracts` GROUP BY contract_date) AS contracts_counts ON groupal_session_assistants.date = contracts_counts.date ORDER BY groupal_session_assistants.date ASC;"))
#       st.write(df_conversion)
