...
@st.cache_data(ttl=600, experimental_allow_widgets=True)
        ...
        if selected_username is not None:
            ...
            if cancel_task_button:
                if selected_task_description is not None:
                    ...
                    today = datetime.date.today()
                    today_str = today.strftime("%Y-%m-%d")
                    uc.run_query(f"UPDATE `company-data-driven.{project_name}.tasks` SET status = 'canceled', canceled_date = '{today_str}', task_cancelator_id = {user_id} WHERE id = {selected_task_id};")
