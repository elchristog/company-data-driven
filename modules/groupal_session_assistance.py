...
def groupal_session_team_member_performance(user_id, project_name):
    ...
    team_member_contacts_week = team_member_contacts[(team_member_contacts["year_attempt_date"] == today.year) & (team_member_contacts["month_attempt_date"] == today.month) & (team_member_contacts["week_attempt_date"] == corrected_week)]
    if len(team_member_contacts_week) < 1 or team_member_contacts_week is None < 0:
        ...
        col1.metric(label="# Week Tests", value = team_member_contacts_week.shape[0])
        # col2.metric(label="# Avg score", value = round(team_member_contacts_week.score.mean(),0))
