...
def groupal_session_absents_and_opportunities(project_name):
    ...
    with col1:
        st.write("### Absents")
        absents_df = uc.run_query_instant(f"SELECT CONCAT(tawl.phone_indicator ,tawl.phone_number) AS phone_number, (COALESCE(DATE_DIFF(CURRENT_DATE(), last_contact_df.last_contact_date, DAY), 99999) - CASE WHEN last_contact_df.last_user_status LIKE '%15%' THEN 15 WHEN last_contact_df.last_user_status LIKE '%30%' THEN 30 WHEN last_contact_df.last_user_status LIKE '%60%' THEN 60 ELSE 0 END) AS days_since_last_contact, DATE_DIFF(CURRENT_DATE(), days_absent_df.last_status_date, DAY) AS days_absent, ppmp.predicted_target_contract_thresholded, ppmp.prob FROM ( SELECT traffic_analytics_whatsapp_lead_id, status AS last_status, meeting_date AS last_status_date, ROW_NUMBER() OVER(PARTITION BY traffic_analytics_whatsapp_lead_id ORDER BY meeting_date DESC) AS meeting_number FROM `company-data-driven.{project_name}.traffic_analytics_groupal_session_assistance`) AS days_absent_df INNER JOIN `company-data-driven.{project_name}.traffic_analytics_whatsapp_leads` AS tawl ON days_absent_df.traffic_analytics_whatsapp_lead_id = tawl.id LEFT OUTER JOIN (SELECT traffic_analytics_whatsapp_leads_id, user_status AS last_user_status, contact_date AS last_contact_date FROM(SELECT tagsc.traffic_analytics_whatsapp_leads_id, ROW_NUMBER() OVER(PARTITION BY tagsc.traffic_analytics_whatsapp_leads_id ORDER BY tagsc.contact_date DESC) AS last_contact_row_number, tagsc.user_status, tagsc.contact_date FROM `company-data-driven.{project_name}.traffic_analytics_groupal_session_crm` AS tagsc)  WHERE last_contact_row_number = 1) AS last_contact_df ON days_absent_df.traffic_analytics_whatsapp_lead_id = last_contact_df.traffic_analytics_whatsapp_leads_id LEFT JOIN `company-data-driven.{project_name}.purchase_propension_model_predictions` AS ppmp ON tawl.id = ppmp.traffic_analytics_whatsapp_leads_id WHERE days_absent_df.meeting_number = 1 AND last_status = 'absent' AND (last_contact_df.last_user_status LIKE '%active%' OR last_contact_df.last_user_status  IS NULL) AND (COALESCE(DATE_DIFF(CURRENT_DATE(), last_contact_df.last_contact_date, DAY), 99999) - CASE WHEN last_contact_df.last_user_status LIKE '%15%' THEN 15 WHEN last_contact_df.last_user_status LIKE '%30%' THEN 30 WHEN last_contact_df.last_user_status LIKE '%60%' THEN 60 ELSE 0 END) > 3 ORDER BY ppmp.prob DESC, days_since_last_contact DESC LIMIT 15;")
        if len(absents_df) < 1:
          st.warning("Waiting for data")
        else:
          st.table(absents_df)
    with col2:
        ...
        never_attended_df = uc.run_query_instant(f'''SELECT CONCAT(tawl.phone_indicator, tawl.phone_number) AS phone_number, (COALESCE(DATE_DIFF(CURRENT_DATE(), last_contact_df.last_contact_date, DAY), 99999) - CASE WHEN last_contact_df.last_user_status LIKE '%15%' THEN 15 WHEN last_contact_df.last_user_status LIKE '%30%' THEN 30 WHEN last_contact_df.last_user_status LIKE '%60%' THEN 60 ELSE 0 END) AS days_since_last_contact, DATE_DIFF(CURRENT_DATE(), tawl.creation_date, DAY) AS days_since_creation, ppmp.predicted_target_contract_thresholded, ppmp.prob FROM `company-data-driven.{project_name}.traffic_analytics_whatsapp_leads` AS tawl LEFT OUTER JOIN (SELECT traffic_analytics_whatsapp_leads_id, user_status AS last_user_status, contact_date AS last_contact_date FROM(SELECT tagsc.traffic_analytics_whatsapp_leads_id, ROW_NUMBER() OVER(PARTITION BY tagsc.traffic_analytics_whatsapp_leads_id ORDER BY tagsc.contact_date DESC) AS last_contact_row_number, tagsc.user_status, tagsc.contact_date FROM `company-data-driven.{project_name}.traffic_analytics_groupal_session_crm` AS tagsc) WHERE last_contact_row_number = 1) AS last_contact_df ON tawl.id = last_contact_df.traffic_analytics_whatsapp_leads_id LEFT JOIN `company-data-driven.{project_name}.purchase_propension_model_predictions` AS ppmp ON tawl.id = ppmp.traffic_analytics_whatsapp_leads_id WHERE (last_contact_df.last_user_status LIKE '%active%' OR last_contact_df.last_user_status IS NULL) AND tawl.id NOT IN (SELECT DISTINCT(traffic_analytics_whatsapp_lead_id) AS whatsapp_id FROM `company-data-driven.{project_name}.traffic_analytics_groupal_session_assistance`) AND (COALESCE(DATE_DIFF(CURRENT_DATE(), last_contact_df.last_contact_date, DAY), 99999) - CASE WHEN last_contact_df.last_user_status LIKE '%15%' THEN 15 WHEN last_contact_df.last_user_status LIKE '%30%' THEN 30 WHEN last_contact_df.last_user_status LIKE '%60%' THEN 60 ELSE 0 END) > 3 ORDER BY ppmp.prob DESC, days_since_last_contact DESC, days_since_creation DESC LIMIT 15;''')
