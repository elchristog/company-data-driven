...
def create_employee_payment(user_id, project_name, project_id):
    ...
    rows = uc.run_query_half_day(f"SELECT u.id, CONCAT(u.name, '_', u.lastname) AS employee_name FROM `company-data-driven.global.users` AS u INNER JOIN `company-data-driven.global.role_assignment` AS ra ON u.id = ra.user_id WHERE project_id = {project_id} AND ra.role_id <> 6 ORDER BY u.id;")
