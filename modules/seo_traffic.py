...
def createPage(project_url_clean):
    ...
    for index, row in df_date.iterrows():
        uc.run_query_insert_update(f"INSERT INTO `company-data-driven.enfermera_en_estados_unidos.traffic_analytics_web_clicks` (date, clicks, impressions, ctr, position) VALUES ('{row['Date']}', {row['Clicks']}, {row['Impressions']}, {row['CTR']}, {row['Position']}) WHERE NOT(date IN (SELECT date FROM `company-data-driven.enfermera_en_estados_unidos.traffic_analytics_web_clicks`));")
