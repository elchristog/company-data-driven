...
def users_to_contact(project_name, user_id):
    ...
    if user_id == 1:
        users_to_contact_df = uc.run_query_instant(f"SELECT u.username, COALESCE(last_contact_df.days_since_last_contact, 999999) AS days_since_last_contact, u_mentor.username AS mentor_username FROM `company-data-driven.{project_name}.contracts` AS c INNER JOIN `company-data-driven.global.users` AS u ON c.user_id = u.id LEFT JOIN (SELECT upsp.contract_id, DATE_DIFF(CURRENT_DATE(), MAX(upsp.creation_date), DAY) AS days_since_last_contact FROM `company-data-driven.{project_name}.user_program_steps_progress` AS upsp GROUP BY upsp.contract_id) AS last_contact_df ON c.id = last_contact_df.contract_id LEFT JOIN `company-data-driven.{project_name}.program_customer_mentor_assignation` AS pcma ON c.user_id = pcma.customer_id INNER JOIN `company-data-driven.global.users` AS u_mentor ON pcma.mentor_id = u_mentor.id WHERE COALESCE(last_contact_df.days_since_last_contact, 999999) > 6 ORDER BY COALESCE(last_contact_df.days_since_last_contact, 999999) DESC;")
    else:
...
def customer_success_crm_add_contact_execution():
    ...
    if 'customer_success_crm_add_contact_selected_username' in st.session_state:
        ...
        if st.session_state.customer_success_crm_add_contact_selected_program_step in ['Aceptación de documentos por la entidad evaluadora', 'Aprobación del examen de inglés', 'Aprobación del examen NCLEX', 'Obtención de oferta de trabajo', 'Inicio del proceso de emigración']:
            idea = 'Entrevista ' + st.session_state.customer_success_crm_add_contact_selected_username + ': ' + st.session_state.customer_success_crm_add_contact_selected_program_step + ' Y asesoria en NCLEX o proceso.'
            uc.run_query_insert_update(f"INSERT INTO `company-data-driven.{st.session_state.customer_success_crm_add_contact_project_name}.content_creation` (id, creation_date, creator_user_id, idea) VALUES (GENERATE_UUID(), CURRENT_DATE(), {st.session_state.customer_success_crm_add_contact_user_id}, '{idea}');") 
...
def customer_success_mentor_assignation_execution():
    ...
    if 'customer_success_mentor_assignation_confirm_choice' in st.session_state:
        if st.session_state.customer_success_mentor_assignation_confirm_choice:
            st.toast("Please wait", icon = "☺️")
            uc.run_query_insert_update(f"INSERT INTO `company-data-driven.{st.session_state.customer_success_mentor_assignation_project_name}.program_customer_mentor_assignation` (id, customer_id, mentor_id, creation_date, creator_user_id) VALUES(GENERATE_UUID(), {st.session_state.customer_success_mentor_assignation_selected_user_id}, {st.session_state.customer_success_mentor_assignation_selected_mentor_id}, CURRENT_DATE(), {st.session_state.customer_success_mentor_assignation_user_id})")
            st.toast("Info saved!", icon = "👾")
            st.balloons()
            time.sleep(1)
            uc.run_query_half_day.clear()
            del st.session_state.customer_success_mentor_assignation_project_name
...
def customer_success_mentor_assignation(user_id, project_name):
    ...
    rows = uc.run_query_instant(f"SELECT u.username, u.id, c.id AS contract_id, pcma.mentor_id FROM `company-data-driven.{project_name}.contracts` AS c INNER JOIN `company-data-driven.global.users` AS u ON u.id = c.user_id LEFT JOIN  `company-data-driven.{project_name}.program_customer_mentor_assignation` AS pcma ON u.id = pcma.customer_id WHERE pcma.mentor_id IS NULL ORDER BY u.username;")
    usernames = []
    contract_ids = []
    user_ids = []
    for row in rows:
    ...
    st.session_state.customer_success_mentor_assignation_project_name = project_name
